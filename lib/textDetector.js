(function initializeTextDetector() {
  ...
  function isCandidateBlock(node, scopeRoot, settings) {
    ...
    if (!scopeRoot.contains(node) || node.hasAttribute("data-cr-processed") || isBlockedContext(node)) {
      return false;
    }
  ...
  function isBlockedContext(node) {
    return node instanceof Element && Boolean(node.closest(BLOCKED_REGION_SELECTOR));
  }
